...
from app.Database.dBService import DBService
# from run import app
...
class IDReaderEndService:
    ...
    def __init__(self, app):
        if not hasattr(self, 'initialized'):
            ...
            with app.app_context():
                self.DBService_Mngr = DBService(app)
            print("HIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIII2")
            print(self.DBService_Mngr)
    ...
    def iDCardreader(self,Imagefile,ext):
        ...
        logger.debug(self.DBService_Mngr.app)
        logger.debug(self.DBService_Mngr.appSubscription)
        self.DBService_Mngr.increment_scan_count()
        logger.debug("Total scans updated successfully in the database")
        return {},None
    
    def iDCardreaderORG(self,Imagefile,ext):
        ...
        # Udate the subscription count
        self.DBService_Mngr.appSubscription.update_total_scans()
        logger.debug("Total scans updated successfully in the database")

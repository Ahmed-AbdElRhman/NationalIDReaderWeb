...
function _show_install_dialog(ProductName, objInstallerUrl, bHTML5, iPlatform, bIE, bSafari, bSSL, strIEVersion) {
	...
	*/
	document.getElementById("dwt-NonInstallContainerID") && (document.getElementById("dwt-NonInstallContainerID").style.display = "none");
	var messageDialog = new ecm.widget.dialog.MessageDialog();
	var packageName = datacapweb.model.util.DCUtil.isMacOS() ? "DynamsoftServiceSetup.pkg" : "DynamsoftServiceSetup.msi";
	messageDialog.showMessage(dojo.string.substitute(datacapweb.resources.dynamsoft_web_twain_not_found_rescan, {InstallerUrl: objInstallerUrl['default'], WebTwainPackage: packageName}));
	messageDialog.setSize(500, 200);
}
...
function OnWebTwainNeedUpgradeCallback(ProductName, objInstallerUrl, bHTML5, iPlatform,
	...
	*/
	...
	var messageDialog = new ecm.widget.dialog.MessageDialog();
	var packageName = datacapweb.model.util.DCUtil.isMacOS() ? "DynamsoftServiceSetup.pkg" : "DynamsoftServiceSetup.msi";
	messageDialog.showMessage(dojo.string.substitute(datacapweb.resources.dynamsoft_web_twain_upgrade_rescan, {InstallerUrl: objInstallerUrl['default'], WebTwainPackage: packageName}));
	messageDialog.setSize(500, 200);
}

...
function updateSubscriptionUI() {
    ...
    //  expiryDate.setDate(expiryDate.getUTCDate() + subscriptionData.expiry_days);
...
async function handleChangePassword(e) {
    ...
    if (!newPassword) {
        showAlert('Please enter the new password', 'warning');
        return;
    }
    ...
    const formData = new FormData();
    formData.append('username',JSON.parse(localStorage.getItem('adminUser')).username);
    formData.append('currentpassword', currentPassword);
    formData.append('newpassword', newPassword);
    console.log("JSON.parse(localStorage.getItem('adminUser')).username)", JSON.parse(localStorage.getItem('adminUser')).username);
    try {
        const response = await fetch(`${API_BASE}/admin/updateuser`, {
            method: 'POST',
            body: formData
        });
        if (!response.ok) {
...
async function renewSubscription() {
    ...
    if (isNaN(expiryDays) || expiryDays < 1 || isNaN(maxScans) || maxScans < 1 || formData === null) {
